    ...
    for( let sy = 0 ; sy  < 2 ; sy++) {
        for( let sx = 0; sx < 2; sx++) {
            ...
            for( let s =0;s<samps;s++){
                ...
                returnVec = returnVec.add(new Vec(clamp(r.x),clamp(r.y),clamp(r.z))).mul(0.25);
            }
        }
    }
return returnVec;
...
const stream = file.createWriteStream("my_file1.ppm");
